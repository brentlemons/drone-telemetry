...
void loop() {
  ...
  double roll = event.orientation.z;
...
//  delay(100);
  ...
  char radiopacket[20]; // = GPS.latitude + "n"; //"Hello World #      ";
  gcvt(GPS.latitude, 8, radiopacket);
  radiopacket[19] = 0;

  JsonObject& gps = root.createNestedObject("gps");
  JsonArray& data = gps.createNestedArray("data");
  data.add(GPS.latitudeDegrees);
  data.add(GPS.longitudeDegrees);
